...
/* usa uma lista duplamente vinculada para percorrer os itens na mesma ordem em que foram adicionados. LinkedHashMap: Diferente de um HashMap comum, ele mantém a ordem em que os itens foram inseridos.*/
import java.util.LinkedHashMap;

/* é uma interface no Java que representa estrutura de dados de chave-valor (mapeamento), onde cada chave única está vinculada a um valor específico*/
...
/* cedulasDisponiveis: Um array fixo que define quais notas o banco aceita (de 100 a 2). Lógica Algorítmica (Sempre com as maiores notas possíveis) Isso é crucial aqui para que o sistema sempre tente usar as notas maiores primeiro.*/
...
/* Valor padrão inicial ele para de funcionar por segurança: Define uma cota mínima (R$ 100,00). Se o caixa tiver menos que isso no total. A "chave" é o valor da nota (ex: 100) e o "valor" é quantas notas existem (ex: 10).*/
        this.cotaMinima = 100.0;  
    }

/* Reposição: São os métodos para quem faz a manutenção do caixa: de uma nota ao que já existe no estoque. reporNotas: Adiciona uma quantidade específica de uma nota ao que já existe no estoque*/
...
/* Módulo do Administrador: Valor Total: getValorTotalDisponivel: Soma tudo o que há no caixa (valor×quantidade) para saber o montante total.*/
...
/* O Cálculo das Notas: Ele cria um mapa temporário (notasParaEntregar). Ele percorre as notas disponíveis (da maior para a menor).*/
        Map<Integer, Integer> notasParaEntregar = new LinkedHashMap<>();
        int restante = valorSaque;
        
        for (int nota : cedulasDisponiveis) {
            int qtdDisponivel = estoqueNotas.get(nota);
/* A conta mágica: restante / nota. Se você quer R$ 130 e a nota é 100, o resultado é 1. Ele verifica se tem essa 1 nota no estoque.*/
